...
def load_data():
    ...
    try:
        ...
        # MEMORY OPTIMIZATION: Convert to categorical during initial load
        # This is more efficient than converting later
...
def compute_metrics(flights):
    ...
    # Use shared constants from module level
    ...
    # MEMORY OPTIMIZATION: helper columns for vectorized aggregation
    # Use constants for delay threshold
    flights['is_on_time'] = ((flights['ARRIVAL_DELAY'] <= DELAY_THRESHOLD_MINUTES) & (flights['CANCELLED'] == 0)).fillna(False).astype(int)
    flights['is_delayed'] = (flights['ARRIVAL_DELAY'] > DELAY_THRESHOLD_MINUTES).fillna(False).astype(int)
    ...
    delay_means_month['Month'] = delay_means_month['MONTH'].map(MONTH_MAP)
    ...
    avg_delay_airline_month['Month'] = avg_delay_airline_month['MONTH'].map(MONTH_MAP)
    ...
    month_stats['Month Name'] = month_stats['MONTH'].map(MONTH_MAP)
    ...
    delay_stream['Month'] = delay_stream['MONTH'].map(MONTH_MAP)
    ...
    vol_month['Month Name'] = vol_month['MONTH'].map(MONTH_MAP)
    ...
    # Delay trends
    monthly_stats = flights.groupby('MONTH')['DEPARTURE_DELAY'].apply(lambda x: (x > DELAY_THRESHOLD_MINUTES).mean() * 100).reset_index(name='Delayed_Pct')
    ...
    if len(bos_flights) > 0:
        bos_stats = bos_flights.groupby('MONTH')['DEPARTURE_DELAY'].apply(lambda x: (x > DELAY_THRESHOLD_MINUTES).mean() * 100).reset_index(name='Delayed_Pct')
        bos_stats['Type'] = 'Boston (BOS)'
        bos_stats['Month'] = bos_stats['MONTH'].map(MONTH_MAP)
    ...
    # MEMORY OPTIMIZATION: Clean up temporary columns
    flights.drop(columns=['is_on_time', 'is_delayed'], inplace=True, errors='ignore')
...
def prepare_ml_data(_flights):
    ...
    # Create target variable: 1 if delayed (>DELAY_THRESHOLD_MINUTES min), 0 otherwise
    ml_df['DELAYED'] = (ml_df['ARRIVAL_DELAY'].fillna(0) > DELAY_THRESHOLD_MINUTES).astype(int)
...
def load_trained_model_v2():
    """
    Load pre-trained model from disk using joblib (supports compression).
