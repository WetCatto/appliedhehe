...
with tab_about:
    # SVG Icons
    svg_check = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#22c55e" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>'

    # Compact Header
    st.markdown("""
    <div style="text-align: center; padding: 16px 0 24px 0;">
        <h2 style="margin: 0 0 4px 0; font-weight: 600; color: #f8fafc;">Airline Performance Dashboard</h2>
        <p style="color: #94a3b8; margin: 0; font-size: 14px;">2015 US Flight Delays Analysis</p>
    </div>
    """, unsafe_allow_html=True)
    
    # Combined Dataset Info + Stats (2 columns)
    st.markdown("""
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 24px;">
        <!-- Left: Dataset Source -->
        <div style="background: linear-gradient(135deg, #1e3a5f 0%, #1e3a8a 100%); border-radius: 8px; padding: 20px;">
            <div style="font-weight: 600; font-size: 15px; color: #f8fafc; margin-bottom: 8px;">Dataset Source</div>
            <div style="font-size: 13px; color: #cbd5e1; margin-bottom: 12px;">2015 Flight Delays Dataset from Kaggle (US DOT)</div>
            <a href="https://www.kaggle.com/datasets/usdot/flight-delays" target="_blank" style="display: inline-block; background-color: #0ea5e9; color: white; padding: 6px 12px; border-radius: 4px; text-decoration: none; font-weight: 500; font-size: 12px;">View on Kaggle</a>
        </div>
        <!-- Right: Key Stats -->
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
            <div style="border: 1px solid #1e293b; border-radius: 8px; padding: 16px; text-align: center; background: #0f172a;">
                <div style="font-size: 22px; font-weight: 700; color: #0ea5e9;">5.8M</div>
                <div style="font-size: 11px; color: #94a3b8;">Flights</div>
            </div>
            <div style="border: 1px solid #1e293b; border-radius: 8px; padding: 16px; text-align: center; background: #0f172a;">
                <div style="font-size: 22px; font-weight: 700; color: #f8fafc;">14</div>
                <div style="font-size: 11px; color: #94a3b8;">Airlines</div>
            </div>
            <div style="border: 1px solid #1e293b; border-radius: 8px; padding: 16px; text-align: center; background: #0f172a;">
                <div style="font-size: 22px; font-weight: 700; color: #0ea5e9;">322</div>
                <div style="font-size: 11px; color: #94a3b8;">Airports</div>
            </div>
            <div style="border: 1px solid #1e293b; border-radius: 8px; padding: 16px; text-align: center; background: #0f172a;">
                <div style="font-size: 22px; font-weight: 700; color: #f59e0b;">2015</div>
    ...
    # Two-column layout: Features + Optimizations
    st.markdown(f"""
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 24px;">
        <!-- Left: Dataset Features -->
        <div style="border: 1px solid #1e293b; border-radius: 8px; padding: 20px; background: #0f172a;">
            <div style="font-weight: 600; color: #f8fafc; margin-bottom: 12px; font-size: 14px;">Dataset Features</div>
            <div style="display: flex; flex-direction: column; gap: 8px; font-size: 13px;">
                <div style="display: flex; align-items: center; gap: 8px; color: #cbd5e1;">{svg_check} <span><strong style="color: #3b82f6;">14 Airlines</strong> – United, American, Delta, Southwest, etc.</span></div>
                <div style="display: flex; align-items: center; gap: 8px; color: #cbd5e1;">{svg_check} <span><strong style="color: #3b82f6;">5 Delay Types</strong> – Airline, Weather, Security, Aircraft, System</span></div>
                <div style="display: flex; align-items: center; gap: 8px; color: #cbd5e1;">{svg_check} <span><strong style="color: #3b82f6;">322 Airports</strong> – All US states and territories</span></div>
                <div style="display: flex; align-items: center; gap: 8px; color: #cbd5e1;">{svg_check} <span><strong style="color: #3b82f6;">Full Metrics</strong> – Departure, Arrival, Distance, Taxi Times</span></div>
            </div>
        </div>
        <!-- Right: Performance Optimization -->
        <div style="border: 1px solid #1e293b; border-radius: 8px; padding: 20px; background: #0f172a;">
            <div style="font-weight: 600; color: #f8fafc; margin-bottom: 12px; font-size: 14px;">Performance Optimization</div>
            <div style="display: flex; flex-direction: column; gap: 8px; font-size: 13px;">
                <div style="display: flex; align-items: center; gap: 8px; color: #cbd5e1;">{svg_check} <span><strong style="color: #22c55e;">85% smaller</strong> – Parquet compression (500MB → 74MB)</span></div>
                <div style="display: flex; align-items: center; gap: 8px; color: #cbd5e1;">{svg_check} <span><strong style="color: #3b82f6;">Pre-computed metrics</strong> – Cloud deployment optimized</span></div>
                <div style="display: flex; align-items: center; gap: 8px; color: #cbd5e1;">{svg_check} <span><strong style="color: #3b82f6;">Pre-trained model</strong> – No training spike on load</span></div>
                <div style="display: flex; align-items: center; gap: 8px; color: #cbd5e1;">{svg_check} <span><strong style="color: #3b82f6;">Session caching</strong> – Instant tab switching</span></div>
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    # ML Model Section Header
    st.markdown("""
    <div style="text-align: center; padding: 12px 0 16px 0;">
        <h3 style="margin: 0 0 4px 0; font-weight: 600; color: #f8fafc;">Machine Learning Model</h3>
        <p style="color: #94a3b8; margin: 0; font-size: 13px;">Random Forest Classifier for Delay Prediction</p>
    </div>
    """, unsafe_allow_html=True)
    
    # ML Stats + Feature Engineering (2 columns)
    st.markdown(f"""
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 24px;">
        <!-- Left: Model Stats -->
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
            <div style="border: 1px solid #1e293b; border-radius: 8px; padding: 16px; text-align: center; background: #0f172a;">
                <div style="font-size: 20px; font-weight: 700; color: #3b82f6;">150</div>
                <div style="font-size: 11px; color: #94a3b8;">Estimators</div>
            </div>
            <div style="border: 1px solid #1e293b; border-radius: 8px; padding: 16px; text-align: center; background: #0f172a;">
                <div style="font-size: 20px; font-weight: 700; color: #f8fafc;">14</div>
                <div style="font-size: 11px; color: #94a3b8;">Max Depth</div>
            </div>
            <div style="border: 1px solid #1e293b; border-radius: 8px; padding: 16px; text-align: center; background: #0f172a;">
                <div style="font-size: 20px; font-weight: 700; color: #22c55e;">250K</div>
                <div style="font-size: 11px; color: #94a3b8;">Training Records</div>
            </div>
            <div style="border: 1px solid #1e293b; border-radius: 8px; padding: 16px; text-align: center; background: #0f172a;">
                <div style="font-size: 20px; font-weight: 700; color: #f59e0b;">~66%</div>
                <div style="font-size: 11px; color: #94a3b8;">F1 Score</div>
            </div>
        </div>
        <!-- Right: Feature Engineering -->
        <div style="border: 1px solid #1e293b; border-radius: 8px; padding: 20px; background: #0f172a;">
            <div style="font-weight: 600; color: #f8fafc; margin-bottom: 12px; font-size: 14px;">Feature Engineering</div>
            <div style="display: flex; flex-direction: column; gap: 8px; font-size: 13px;">
                <div style="display: flex; align-items: center; gap: 8px; color: #cbd5e1;">{svg_check} <span><strong style="color: #3b82f6;">Target Encoding</strong> – Risk scores per route</span></div>
                <div style="display: flex; align-items: center; gap: 8px; color: #cbd5e1;">{svg_check} <span><strong style="color: #3b82f6;">Temporal Features</strong> – Time-of-day, weekends</span></div>
                <div style="display: flex; align-items: center; gap: 8px; color: #cbd5e1;">{svg_check} <span><strong style="color: #3b82f6;">Balanced Sampling</strong> – 50/50 delayed/on-time</span></div>
                <div style="display: flex; align-items: center; gap: 8px; color: #cbd5e1;">{svg_check} <span><strong style="color: #3b82f6;">Operational Metrics</strong> – Distance, taxi-out</span></div>
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    
    # Tech Stack (compact single row)
    st.markdown("""
    <div style="border: 1px solid #1e293b; border-radius: 8px; padding: 16px 20px; background: #0f172a;">
        <div style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 16px;">
            <div>
                <div style="font-weight: 600; color: #94a3b8; font-size: 11px; text-transform: uppercase; margin-bottom: 6px;">Frontend</div>
                <div style="color: #f8fafc; font-size: 13px;">Streamlit • Plotly</div>
            </div>
            <div>
                <div style="font-weight: 600; color: #94a3b8; font-size: 11px; text-transform: uppercase; margin-bottom: 6px;">Data</div>
                <div style="color: #f8fafc; font-size: 13px;">Pandas • NumPy • Parquet</div>
            </div>
            <div>
                <div style="font-weight: 600; color: #94a3b8; font-size: 11px; text-transform: uppercase; margin-bottom: 6px;">ML</div>
                <div style="color: #f8fafc; font-size: 13px;">Scikit-learn • Random Forest</div>
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)
